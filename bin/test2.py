...
def main(args):
    ...
    cfg.n_clusters += 1
    print("test point 1 line 60")
    while True:
        print("test point 2 line 62")
        try:
            # data pipeline
            data_pipeline = DataPipeline(args.dataset, config=cfg, is_training=False)
            print(data_pipeline)
            print("test point 3 line 66")
            samples = {
            ...
            with tf.compat.v1.Session() as sess:
                ...
                step = tf.compat.v1.train.global_step(sess, model.global_step)
                mean_metrics = {}
                for key in metrics:
                    mean_metrics[key] = 0
                ...
                while True:
                    try:
                        ...
                        print
                        'Evaluation completed ({} epochs).'.format(cfg.n_epochs)
                        print
                        "{} windows seen".format(n)
                        break
            ...
            print
            'Sleeping for {}s'.format(args.eval_interval)
            time.sleep(args.eval_interval)
